...
import { useNavigate, Link,useParams, useRoutes, useLocation } from 'react-router-dom';
...
<div className={styles.blogContainer2}>
  ...
  <div>
    ...
    <div onClick={()=> Router("/mailVerification/fr",{state: {ip:ip,uid:uid}})} style={{color:"red",textDecoration:"none",paddingLeft:"12px",cursor:"pointer"}} shallow><p>Pas accès a votre mobile? Confirmer par e-mail</p></div>
    </div>
    ...
    <div className={styles.content}>
      <div className={styles.contentBox2}>
        ...
        {!show && (<>
          ...
          <p className={styles.message2}>Le code de vérification expirera dans 3 minutes.</p>
        </>)}
        {show && ( <p className={styles.message}>Le code a expiré. Veuillez saisir le nouveau code de securite qui vient d'etre envoye sur votre boite sms</p> )}
        <br />
        <p style={{color:"orangered",fontWeight:"bold"}}>{0}{min}:{sec1 < 10 && "0"}{sec1}</p><br />
        <input value={code} onChange={(e)=> setCode(e.target.value)} placeholder='Code de verification' type="text" required/>
      </div>
